...
## subcortical volume
```{r Select subcortical regions from Amir's dataset, eval=FALSE, include=FALSE}
# These data have already gone through z-score standardisation and mean+-5SD outlier removal.
...
# !! Subcortical phenotype data right now only includes volume !!
...
```{r Subcortical phenotype data standardisation and outlier removal}
...
subcor.formatted.mad = fread(here("imaging_data/subcortical_formatted.txt"))
list.no_outlier = list() # List to find all phenotype with no outliers


for (i in 3:ncol(subcor.formatted.mad)) { # FID and IID are the first two columns
  col_name = colnames(subcor.formatted.mad)[i]
  col_mad = mad(subcor.formatted.mad[[col_name]], na.rm = TRUE)
  col_mad_outlier_n = sum(abs(subcor.formatted.mad[[col_name]]) > col_mad * 5, na.rm = TRUE)
  
  if (col_mad_outlier_n != 0) {
    outlier_row_n = which(abs(subcor.formatted.mad[[col_name]]) > col_mad * 5)
    subcor.formatted.mad[[col_name]][abs(subcor.formatted.mad[[col_name]]) > 5 * col_mad] <- NA # In column col_name, if |value| > 5*column mad SD, then |value| is outlier
    print(paste0("The detected outlier number is ", col_mad_outlier_n, 
                 ". At column ", col_name, 
                 ", now removing the following rows ", 
                 paste(outlier_row_n, collapse = ", ")))
  } else {
    print(paste0("No outlier detected at ", col_name, ". Moving on..."))
    list.no_outlier = append(list.no_outlier, col_name)
  }
}

# phenotype > 50 mad outliers:  Lateral.Ventricle(230); Inf.Lat.Vent(610); vessel(642); X4th.Ventricle(57); WM.hypointensities(3010); CC_Central(229); CC_Mid_Anterior(108)


print(paste0("Phenotypes that had no outliers are ", paste(list.no_outlier, collapse = ", "))) # report no outlier phenotypes
write.table(subcor.formatted.mad, file = here("imaging_data/subcortical_formatted.mad.txt"), row.names = F, col.names = T, quote = F)


# Generate histogram in pdf for double-checking
subcor.formatted = fread(here("imaging_data/subcortical_formatted.txt")) # non-median outlier corrected table

subcor_plot_path = paste0("/rds/user/yg330/rds-genetics_hpc-Nl99R8pHODQ/UKB/Imaging_genetics/yg330/GWAS_Trait/hist_plots/subcor.pdf")
subcor_plot.mad_path = paste0("/rds/user/yg330/rds-genetics_hpc-Nl99R8pHODQ/UKB/Imaging_genetics/yg330/GWAS_Trait/hist_plots/subcor.mad.pdf")

for (i in 1) { # unclear but when put into a loop the margin problem was fixed.
  pdf(file = subcor_plot_path)
    title = "subcor"
    hist(subcor.formatted)
  dev.off()
  
    pdf(file = subcor_plot.mad_path)
    title = "subcor.mad"
